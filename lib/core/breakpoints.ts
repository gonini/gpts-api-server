...
export function detectBreakpoints(rows: EarningsRow[]): Breakpoint[] {
  ...
  for (let i = 0; i < sortedRows.length; i++) {
    ...
    let epsYoY_NM = false;
    if (current.eps !== null && previous.eps !== null) {
      if (Math.abs(previous.eps) < 0.1) {
        epsYoY_NM = true;
        console.log(
          `EPS YoY marked NM due to small previous EPS (${previous.eps}).`
        );
      } else if ((current.eps >= 0) !== (previous.eps >= 0)) {
        epsYoY_NM = true;
    ...
    if (current.revenue !== null && previous.revenue !== null) {
      if (previous.revenue === 0) {
        revYoY_NM = true;
        console.log('Revenue YoY marked NM due to zero previous revenue.');
      } else if ((current.revenue >= 0) !== (previous.revenue >= 0)) {
        revYoY_NM = true;
