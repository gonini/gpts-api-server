...
/**
 * Analyze API 로컬 테스트 스크립트
 * 
 * 사용법:
 * node test-analyze-api.js [ticker] [from] [to]
 * 
 * 예시:
 * node test-analyze-api.js AAPL 2014-01-01 2016-12-31
 * node test-analyze-api.js MSFT 2020-01-01 2024-12-31
 */
...
function testAnalyzeAPI() {
  return new Promise((resolve, reject) => {
    const url = `http://localhost:${port}/api/analyze?ticker=${ticker}&from=${from}&to=${to}`;
...
async function runMultipleTests() {
  ...
  for (const testCase of testCases) {
    ...
    try {
      // 각 테스트 케이스에 맞는 파라미터로 API 호출
      const originalTicker = ticker;
      const originalFrom = from;
      const originalTo = to;
      
      // 전역 변수 임시 변경
      global.ticker = testCase.ticker;
      global.from = testCase.from;
      global.to = testCase.to;
      ...
      // 원래 값 복원
      global.ticker = originalTicker;
      global.from = originalFrom;
      global.to = originalTo;
    } catch (error) {
...
사용법:
  node test-analyze-api.js [ticker] [from] [to] [port]
...
예시:
  node test-analyze-api.js AAPL 2014-01-01 2016-12-31
  node test-analyze-api.js --multiple
  node test-analyze-api.js MSFT 2020-01-01 2024-12-31 3002
